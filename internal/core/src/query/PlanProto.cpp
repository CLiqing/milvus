...
ProtoParser::ParseSearchInfo(const planpb::VectorANNS& anns_proto) {
    ...
    // `filter_result_representation=sparse` is a runtime *preference*: the
    // filter tries to emit a Sparse valid-ID list and falls back to Dense when
    // the list would exceed the cap or the producer is unsupported.  The
    // actual representation is decided by the filter and carried by the
    // valid-ID payload, and VectorSearchNode aligns the Cardinal scan mode
    // (valid_ids_per_query vs auto) to it at runtime, so it is NOT translated
    // into a hard bf_filter_scan_mode here.
