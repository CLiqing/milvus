...
struct ArithOpElementFunc {
    ...
    operator()(const T* src,
        ...
        // Reuse the baseline bitset scalar kernel for gathered offsets;
        // only the access pattern differs, not the arithmetic semantics.
        if constexpr (filter_type == FilterType::random) {
            if constexpr (!std::is_same_v<decltype(CmpOpHelper<cmp_op>::op), void> &&
                          !std::is_same_v<decltype(ArithOpHelper<arith_op>::op), void>) {
                using Kernel = bitset::ArithCompareOperator<
                    ArithOpHelper<arith_op>::op, CmpOpHelper<cmp_op>::op>;
                for (size_t i = 0; i < size; ++i) {
                    const auto offset = offsets ? offsets[i] : i;
                    res[i] = Kernel::compare(src[offset], right_operand, val);
                }
                return;
            } else {
                ThrowInfo(UnexpectedError, "unsupported arithmetic/comparison");
            }
        }
...
struct ArithOpIndexFunc {
    ...
    operator()(Index* index,
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            if constexpr (!std::is_same_v<decltype(CmpOpHelper<cmp_op>::op), void> &&
                          !std::is_same_v<decltype(ArithOpHelper<arith_op>::op), void>) {
                res[i] = bitset::ArithCompareOperator<ArithOpHelper<arith_op>::op,
                    CmpOpHelper<cmp_op>::op>::compare(raw.value(), right_operand, val);
            } else {
                ThrowInfo(UnexpectedError, "unsupported arithmetic/comparison");
            }
        }
 ...
 public:
    ...
    DescribeFilterSource() const override {
        return DescribeColumnFilterSource(
            ...
            expr_->op_type_, expr_->arith_op_type_);
    }
