...
PhyBloomFilterExpr::ExecVisitorImplJson(EvalCtx& context) {
    ...
    auto execute_sub_batch =
        ...
        for (int i = 0; i < size; ++i) {
            ...
            // (SplitBlockBloomFilterView::TestBytes / TestInt64), so a JSON
            // string cannot alias an int64-only filter and vice versa. Nothing
            // is needed here: a domain the blob never recorded simply returns
            // false, which is the correct answer — no value of that type was
            // ever a member.
