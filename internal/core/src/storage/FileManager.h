 ...
 public:
    ...
    GetRemoteTextLogFilePrefixV2() const {
        ...
        boost::filesystem::path path1 =
            ...
            std::to_string(field_meta_.segment_id) + "/" +
            std::to_string(field_meta_.field_id);
        std::string v1_prefix = NormalizePath(prefix / path / path1);
        if (bucket.empty()) {
            return v1_prefix;
        }
        return NormalizePath(bucket / v1_prefix);
    ...
    std::string
    GetStorageV2TextLogPrefix() const {
        if (fs_->type_name() == "local") {
            return GetRemoteTextLogPrefixV2();
        }
        return GetRemoteTextLogFilePrefixV2();
    }
